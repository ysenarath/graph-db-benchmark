...
| Shortest Path | 11,715 ms | **4,165 ms** | 7,183 ms |
| Temporal Filter (3-hop) | **25 ms** | 241 ms | 3,025 ms |
| Pattern Match (A→B→C) | **28 ms** | 154 ms | 6 ms |
| **Bulk Ingest** | **6.7 s** | 17.9 s | 23.0 s |
...
| Temporal filtering on edges | **DuckDB** — secondary index on `ts`, 25 ms vs 3 s at large scale |
| Shortest path | **LadybugDB** — fastest of the three |
...
CozoDB's temporal filter is 123× slower than DuckDB on the large dataset. This is structural: CozoDB has no secondary index on `ts`, so any timestamp range filter requires a full edge scan. Investigated and confirmed not a fairness bug — see [`results/RESULTS.md`](results/RESULTS.md).

## Reproduce

```bash
git clone https://github.com/ysenarath/graph-db-benchmark
cd graph-db-benchmark/graph-db-benchmark
uv sync
...
- **LadybugDB `shortestPath()`** not available in v0.18. Used `MATCH p = (a)-[:Edge*1..8]->(b) RETURN length(p) ORDER BY length(p) LIMIT 1` instead.
